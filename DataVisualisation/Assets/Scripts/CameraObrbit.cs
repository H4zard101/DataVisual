    ...
    void Update()
    {
        if (!UIHelper.instance.IsPointerOverUIElement())
        {
            ...
            if (down)
            {
                ...
                target.x = targetOnDown.x + (mouse.x - mouseOnDown.x) * 0.00005f * zoomDamp;
                target.y = targetOnDown.y + (mouse.y - mouseOnDown.y) * 0.00005f * zoomDamp;
            ...
            distance += (distanceTarget - distance) * 0.03f;
            Vector3 position;
